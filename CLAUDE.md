...
- **`src/config/waveConfig.ts`** - Central configuration for all wave parameters. Contains `WaveConfig` interface, `defaultWaveConfig`, `wavePresets` (calm/moderate/big/extreme), and `configLimits` for UI sliders. This is the single source of truth for wave behavior.
...
The `WaveConfig` object flows down from App. ControlPanel updates trigger re-renders, and WaveMesh reads config values each frame in its `useFrame` loop.
